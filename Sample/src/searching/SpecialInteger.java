...
public class SpecialInteger {
	...
	private static boolean isPossible(int[] a, int key, int max) {
		for (int i = 0; i <= a.length - key; i++) {
			int keyCount = key;
			int ii = i;
			int sum = 0;
			while (keyCount != 0 && ii < a.length) {
				sum = sum + a[ii];
				if (sum > max) {
					return false;
				}
	...
	private static boolean isPossible1(int[] a, int key, int max) {
		...
		for (int i = 1; i <= a.length - key; i++) {
			
			sum = sum -a[i-1]+a[i+key-1];
			if(sum > max) {
				return false;
			}
